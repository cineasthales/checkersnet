...
class Game:
		...
		for x in xrange(8):
			for y in xrange(8):
				if recebe[i] == '0':
					...
					self.board.matrix[x][y].occupant = Piece(BLUE)
					self.board.matrix[x][y].occupant.color = BLUE
					self.board.matrix[x][y].occupant.king = False
				elif recebe[i] == '2':
					self.board.matrix[x][y].occupant = Piece(RED)
					self.board.matrix[x][y].occupant.color = RED
					self.board.matrix[x][y].occupant.king = False
				elif recebe[i] == '3':
					self.board.matrix[x][y].occupant = Piece(BLUE)
					self.board.matrix[x][y].occupant.color = BLUE
					self.board.matrix[x][y].occupant.king = True
				elif recebe[i] == '4':
					self.board.matrix[x][y].occupant = Piece(RED)
					self.board.matrix[x][y].occupant.color = RED
					self.board.matrix[x][y].occupant.king = True
	...
	def end_turn(self):
		...
		if self.check_for_endgame():
			self.graphics.draw_message("VITORIA!")
			#if self.turn == BLUE:
			...
			self.update()
			self.graphics.draw_message("AGUARDE!")
			self.update()
			self.updateFromServer()
		...
		for x in xrange(8):
			for y in xrange(8):
				if self.board.matrix[x][y].occupant == None:
					turno = turno + '0'
				elif self.board.matrix[x][y].occupant.color == BLUE:
					if self.board.matrix[x][y].occupant.king == False:
						turno = turno + '1'
					else:
						turno = turno + '3'
